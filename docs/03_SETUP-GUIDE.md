...
3. Choose **Public** or **Private** — both work fine with Azure Static
   Web Apps.
...
> [!WARNING]
> Choose public vs. private deliberately — don't default to whichever
> GitHub suggests. A **public** repository means anyone on the internet
> can view **every file and the entire commit history**, not just the
> current version. That's usually fine for this project (it's just
> HTML/CSS/JS, no secrets — deployment credentials live in GitHub's
> encrypted repo secrets, not your code), but it should be an active
> decision, not an accident. While you're at it, **check your GitHub
> account's existing repositories and settings too**, in case anything
> else there should be private and currently isn't.
